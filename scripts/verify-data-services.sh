...
actual="$(git ls-remote "$OPENEVEREST_REPO" | awk -v sha="$OPENEVEREST_COMMIT" '$1==sha {print $1; exit}')"
...
grep -A2 'rbac:' apps/data-services/openeverest-values.yaml | grep -q 'enabled: true'
grep -A2 'tls:' apps/data-services/openeverest-values.yaml | grep -q 'enabled: true'
grep -A3 'dbNamespace:' apps/data-services/openeverest-values.yaml | grep -q 'namespaceOverride: layersentry-dbaas'
...
chart_version="$(awk '$1=="version:" {gsub(/\"/,"",$2); print $2; exit}' "$chartdir/charts/everest/Chart.yaml")"
app_version="$(awk '$1=="appVersion:" {gsub(/\"/,"",$2); print $2; exit}' "$chartdir/charts/everest/Chart.yaml")"
